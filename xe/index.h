...
<style>
  ...
  *{box-sizing:border-box} html,body{min-height:100%}
  body{margin:0;font-family:ui-sans-serif,system-ui,Arial;background:radial-gradient(1200px 800px at 50% -10%, #1f2937 0%, var(--bg) 60%);
       color:var(--txt);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:8px;gap:12px}
  ...
  .cam-card{padding:8px;width:min(95vw,100%);max-width:none}
  .cam-container{position:relative;width:100%;padding-top:50%;background:#000;border-radius:8px;overflow:hidden}
...
<body>
  <div class="card cam-card">
    ...
    <div class="cam-container">
      <iframe id="camFrame" src="http://192.168.4.2:81/stream" allow="camera"></iframe>
    </div>
  ...
  <div class="card">
    ...
    <div style="display:flex;gap:12px;align-items:center;padding:12px;background:#0b1220;border:1px solid #1f2937;border-radius:12px;">
      <div style="flex:1">
        <div style="font-size:13px;color:var(--muted);margin-bottom:4px;">Detected Color:</div>
        <div id="colorValue" style="font-size:20px;font-weight:600;">none</div>
      </div>
      <div style="flex:1">
        <div style="font-size:13px;color:var(--muted);margin-bottom:4px;">Last Update:</div>
        <div id="colorAge" style="font-size:14px;">--</div>
      </div>
      <div id="colorIndicator" style="width:48px;height:48px;border-radius:50%;background:#1f2937;border:2px solid #374151;"></div>
    </div>
    <div style="margin-top:10px;font-size:12px;color:var(--muted);text-align:center;">
      <span style="color:#ef4444;">&#11044;</span> Red = Right &nbsp;|&nbsp; 
      <span style="color:#22c55e;">&#11044;</span> Green = Left
    </div>
  </div>
...
<script>
  ...
  function checkCamera() {
    fetch('http://192.168.4.2:81/stream', {method:'HEAD', mode:'no-cors'})
      .then(()=> {
  ...
  async function refreshObstacleStatus(){
    try{
      ...
      if (data.action === 'red_right') {
        statusText += '<div style="margin-top:4px"><span class="red">● Màu ĐỔ</span> → Rẽ <strong>PHẢI</strong></div>';
      } else if (data.action === 'green_left') {
        statusText += '<div style="margin-top:4px"><span class="green">● Màu XANH</span> → Rẽ <strong>TRÁI</strong></div>';
      } else if (data.action === 'unknown_right') {
        statusText += '<div style="margin-top:4px"><span class="yellow">● Không nhận diện</span> → Rẽ <strong>PHẢI</strong> (mặc định)</div>';
      }
  ...
  async function refreshColor(){
    try{
      ...
      // Update color indicator and text color
      if (data.color === 'red') {
        colorIndicator.style.background = 'linear-gradient(135deg, #ef4444, #dc2626)';
        colorIndicator.style.borderColor = '#ef4444';
        colorValue.style.color = '#ef4444';
      } else if (data.color === 'green') {
        colorIndicator.style.background = 'linear-gradient(135deg, #22c55e, #16a34a)';
        colorIndicator.style.borderColor = '#22c55e';
        colorValue.style.color = '#22c55e';
      } else if (data.color === 'yellow') {
        colorIndicator.style.background = 'linear-gradient(135deg, #f59e0b, #d97706)';
        colorIndicator.style.borderColor = '#f59e0b';
        colorValue.style.color = '#f59e0b';
      } else {
